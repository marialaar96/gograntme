...
ALL_GRANTS.forEach(function(element, index) {
    ...
    const content = {title, company, deadline, description, numavailable, availableperperson, expectation, requirements, howtoapply, wheretosend};

    const renderedGrant = pug.renderFile(TEMPLATE_PATH, { content });
    const htmlFileName = element.title + index + ".html";
